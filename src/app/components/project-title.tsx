...
function ProjectTitle({ title }: { title: string }) {
  return (
    <div className="flex items-center sticky md:static w-full top-0 left-0 bg-slate-900 md:bg-black z-[9999] p-4 mb-8">
      <Link href={"/"}>
        <div className="md:fixed md:top-12 md:left-12 text-slate-200 rounded-full border border-white flex justify-center items-center h-8 w-8 cursor-pointer">
          <img src={Back} />
        </div>
      </Link>
      <div className="text-2xl md:text-4xl text-slate-200 mx-auto md:mb-10 md:my-20 text-center">
        {title}
